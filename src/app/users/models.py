...
class User(Model):
    ...
    @classmethod
    async def create_superuser(cls, email, password) -> "User":
        user = cls(email=email, superuser=True, activated=True)
        user.set_password(password)
        await user.save()
        return user
    ...
    def set_unusable_password(
        self, unusable_prefix: str = secrets.UNUSABLE_PASSWORD_PREFIX
    ) -> None:
        self.hashed_password = self._generate_unusable_password(
            unusable_prefix=unusable_prefix
        )

    def _generate_unusable_password(self, unusable_prefix: str):
        return unusable_prefix + self._generate_unusable_suffix()

    def _generate_unusable_suffix(
        self,
        unusable_suffix_len=40,
        signs="abcdefghijklmnopqrstuvwxyz" "ABCDEFGHIJKLMNOPQRSTUVWXYZ" "1234567890",
    ) -> str:
